...
class Boxers(db.Model):
    ...
    def create_boxer(cls, name: str, weight: float, height: float, reach: float, age: int) -> None:
        ...
        if weight < 125:
            ...
            raise ValueError(f"Weight must be at least 125.")
    
        try:
            #check for exisitng boxer with the same name
            existing = Boxers.query.filter_by(name=name.strip()).first()
            if existing:
                logger.error(f"Boxer with name '{name}' already exists.")
                raise IntegrityError(f"Boxer with name '{name}' already exists.", params=None, orig=None)
    ...
    def get_boxer_by_id(cls, boxer_id: int) -> "Boxers":
        ...
        try:
            ...
            logger.info(f"Successfully retrieved boxer: {boxer.name} - {boxer.age}")
            return boxer
        except ValueError as e:
            logger.info(f"Boxer with ID {boxer_id} not found.")
            raise
